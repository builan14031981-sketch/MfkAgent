...
export const MessageList = memo(function MessageList({ messages, timeline, streamingError, isStreaming, reasoningActive, currentAgent, onQuote, onRegenerate, onRetry, onEdit, onApproveApproval, onDenyApproval, onSelectChoice, onChoiceCustomText, onSkipChoice, onActiveUserMessageChange, scrollPersistenceKey }: MessageListProps) {
  ...
  useEffect(() => {
    ...
    // 自动朗读：流式结束（true→false）时，如果开启了自动朗读，朗读最后一条 AI 消息
    if (!isActive && wasActive) {
      const settings = useSettingsStore.getState().settings;
      const ttsEnabled = settings?.tts_enabled !== "false";
      const autoPlay = settings?.tts_auto_play === "true";
      if (ttsEnabled && autoPlay) {
        const lastAssistant = [...messages].reverse().find(
          (m) => m.role === "assistant" && m.content?.trim()
        );
        if (lastAssistant) {
          // 延迟 300ms，确保消息内容已完全更新
          setTimeout(() => playTts(lastAssistant.content, lastAssistant.id), 300);
        }
      }
    }
  }, [isActive, updateJumpButton, messages, playTts]);
      ...
      >
        {isEmptyState ? (
          ...
          <>
            ...
            {timeline.length > 0 && (
              <div style={{ maxWidth: "800px", margin: "0 auto", marginBottom: "12px" }}>
