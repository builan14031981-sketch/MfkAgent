...
export const ChatInput = memo(function ChatInput({
  ...
  const handleScreenshot = async () => {
    ...
    try {
      ...
      // 用 IPC 返回的 base64 data URL 直接创建 File 对象，避免 fetch file:// 被 CSP 阻止
      ...
      const file = new File([u8arr], `screenshot_${Date.now()}.png`, { type: mime });
      // 添加到本地截图预览列表（不立即上传，发送时统一上传）
  ...
  const handleSend = useCallback(async () => {
    ...
    if (screenshots.length > 0) {
      ...
      for (const item of screenshots) {
        try {
          // 用 Promise.resolve 包装，确保 onUploadFile 即使返回 void 也能正确 await
          await Promise.resolve(onUploadFile(item.file));
          uploadSuccessCount++;
        } catch (err) {
      ...
      if (uploadSuccessCount === 0) {
        console.error("[Screenshot] all screenshots failed to upload, abort send");
        // 这里可以加一个 toast 提示，暂时用 console.error
        return;
      }
    ...
    clearDraft();
    onSend();
  }, [canSend, screenshots, clearDraft, onSend, onUploadFile]);
