...
export const useStreamStore = create<StreamStore>()(
  ...
  {
    name: "mfk-task-store",
    // ⚠️ 设计意图：partialize 仅序列化 sessions.tasks（任务面板数据）、sessions.tokenUsage
    // （上下文水位快照，2026-08-12 加入：刷新/重启后恢复上下文仪表盘显示）与
    // sessions.totalCachedTokens/totalPromptTokens（会话级缓存累计，2026-09-03 加入：
    // 修复"重新进会话要发消息平均缓存命中率才重新生效"——此前未持久化导致刷新后归 0）。
    ...
    merge: (persisted, current) => {
      const p = persisted as {
        sessions?: Record<
          string,
          { tasks: TaskNode[]; tokenUsage?: TokenUsageEvent | null; totalCachedTokens?: number; totalPromptTokens?: number }
        >;
      };
      ...
      for (const [chatIdStr, data] of Object.entries(p.sessions)) {
        ...
        const hasCumulative = (data.totalCachedTokens ?? 0) > 0 || (data.totalPromptTokens ?? 0) > 0;
        if (!hasTasks && !hasUsage && !hasCumulative) continue;
        const base = mergedSessions[chatId] ?? createDefaultSession();
        mergedSessions[chatId] = {
          ...base,
          // 旧持久化数据无 tasks/tokenUsage/累计字段时保持内存默认值
          ...
          ...(hasCumulative && base.totalCachedTokens === 0 && base.totalPromptTokens === 0
            ? { totalCachedTokens: data.totalCachedTokens ?? 0, totalPromptTokens: data.totalPromptTokens ?? 0 }
            : {}),
        };
      }
