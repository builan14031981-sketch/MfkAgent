...
模型 ID 命名规范（强制）：
- 官方模型（该模型的原厂 provider）：直接用模型原名，如 qwen-max、glm-5、gpt-4o
- 非官方模型（其他 provider 代理/转售的模型）：必须加 <provider_id>- 前缀，如 qwen-glm-5、siliconflow-deepseek-v4-pro
- 目的：避免不同 provider 的同名模型在 resolve_all() 中互相覆盖，导致有额度的模型被无额度的覆盖而消失
"""
...
PROVIDERS: list[ProviderDef] = [
    ...
    ProviderDef(
        ...
        models=(
            ...
            # 非官方模型（其他厂商模型通过百炼平台提供）：必须加 qwen- 前缀，避免与原厂 provider 同名覆盖
            ProviderModel("qwen-deepseek-r1-distill-qwen-7b", "deepseek-r1-distill-qwen-7b", "DeepSeek R1 蒸馏 7B (百炼)", context_window=131_072),
            ProviderModel("qwen-glm-5", "glm-5", "GLM-5 (百炼)", context_window=128_000),
    ...
    ProviderDef(
        ...
        description="GPT-4o / o3 推理系列，全球最主流模型提供商",
        website="https://platform.openai.com",
        models=(
            ProviderModel("gpt-4o", "gpt-4o", "GPT-4o", context_window=128_000),
            ProviderModel("gpt-4o-mini", "gpt-4o-mini", "GPT-4o Mini", context_window=128_000),
            ProviderModel("o3", "o3", "o3 推理模型", context_window=200_000),
            ProviderModel("o3-mini", "o3-mini", "o3 Mini 推理", context_window=200_000),
        ),
    ),
    ProviderDef(
        id="anthropic",
        ...
        description="Claude Opus/Sonnet/Haiku，代码与长文本能力强",
        website="https://www.anthropic.com",
        models=(
            ProviderModel("claude-opus-4-8", "claude-opus-4-8", "Claude Opus 4.8", context_window=1_048_576),
            ProviderModel("claude-sonnet-5", "claude-sonnet-5", "Claude Sonnet 5", context_window=200_000),
            ProviderModel("claude-sonnet-4-6", "claude-sonnet-4-6", "Claude Sonnet 4.6", context_window=200_000),
            ProviderModel("claude-haiku-4-5", "claude-haiku-4-5", "Claude Haiku 4.5", context_window=200_000),
        ),
    ),
    ProviderDef(
        id="doubao",
        ...
        description="火山引擎豆包大模型，Seed 2.1 系列 Coding/Agent 强项",
        website="https://www.volcengine.com/product/doubao",
        models=(
            ProviderModel("doubao-seed-2-1-pro", "doubao-seed-2-1-pro", "豆包 Seed 2.1 Pro", context_window=256_000),
            ProviderModel("doubao-seed-2-1-turbo", "doubao-seed-2-1-turbo", "豆包 Seed 2.1 Turbo", context_window=256_000),
            ProviderModel("doubao-seed-evolving", "doubao-seed-evolving-latest-version", "豆包 Seed Evolving", context_window=1_048_576),
            ProviderModel("doubao-seed-2-0-lite", "doubao-seed-2-0-lite", "豆包 Seed 2.0 Lite", context_window=128_000),
        ),
    ),
    ProviderDef(
        id="hunyuan",
        ...
        description="腾讯混元大模型，T1 推理 / TurboS 快思考",
        website="https://hunyuan.tencent.com",
        models=(
            ProviderModel("hunyuan-t1", "hunyuan-t1", "混元 T1 深度推理", context_window=256_000),
...
# ── 动态自定义端点（custom_providers 表）──────────────────────────────
# 用户在前端"自定义模型"区添加的 OpenAI 兼容端点，运行时转为 ProviderDef
# 合并进 PROVIDERS，与官方供应商 100% 同构（api_key / enabled_models 等全部复用 settings 表）。

_custom_provider_defs: list[ProviderDef] = []


def _load_custom_providers_from_db() -> list[ProviderDef]:
    """从 custom_providers 表加载所有自定义端点，转为 ProviderDef 列表。"""
    try:
        ...
        try:
            ...
            return [
                ProviderDef(
                    ...
                    env_key="",  # 自定义端点不读 .env，Key 只存 settings 表
                    description=r.description or "用户自定义 OpenAI 兼容端点",
                    category="custom",
                    models=(),  # 初始无预定义模型，用户通过"一键拉取远程模型"添加
                )
                for r in rows
            ]
        finally:
        ...
        # 表不存在或DB未就绪时静默返回空，不影响启动
        import logging
        logging.getLogger(__name__).debug(f"load custom providers skipped: {e}")
        return []


def reload_custom_providers() -> None:
    """重新从 DB 加载自定义端点，合并进 PROVIDERS / PROVIDER_MAP。

    增删改 custom_providers 后调用此函数，运行时生效无需重启。
    """
    global PROVIDERS, PROVIDER_MAP, _custom_provider_defs
    _custom_provider_defs = _load_custom_providers_from_db()
    PROVIDERS = [p for p in PROVIDERS if p.category != "custom"] + _custom_provider_defs
    PROVIDER_MAP = {p.id: p for p in PROVIDERS}


def seed_builtin_custom_providers() -> None:
    """首次启动时写入内置自定义端点模板（如 FreeLLMAPI 本地网关）。

    幂等：已存在则跳过。在 main.py 启动流程、表创建完成后调用。
    """
    try:
        from app.core.database import SessionLocal
        from app.models.agent import CustomProvider
        ...
        try:
            ...
            for b in builtins:
                exists = db.query(CustomProvider).filter(
                    CustomProvider.name == b["name"],
                    CustomProvider.is_builtin.is_(True),
                ).first()
                if not exists:
                    db.add(CustomProvider(
                        name=b["name"],
                        default_api_base=b["default_api_base"],
        ...
        import logging
        logging.getLogger(__name__).warning(f"seed builtin custom providers skipped: {e}")
