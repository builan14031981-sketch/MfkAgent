...
GUIDANCE_TEMPLATES: dict[str, dict] = {
    ...
    "coding": {
        "tool_flow": [
            ...
            "2. 实现/修改阶段：在已有项目修复时遵循最小修改原则；在新建项目/模块时鼓励在单轮中一次性并发写入核心代码与测试骨架",
            "3. 验证阶段：代码写入后统一用 execute_command 编译/运行测试，确保通过",
            "4. 版本管理阶段（推荐）：用 git_status 查看改动 → git_diff 查看差异 → git_commit 提交",
            "5. 同步阶段（可选）：用 git_push 推送提交到远程（需审批）",
        ],
        "suggestions": [
            "修改已有代码前必须先用 read_file 读取目标文件的最新内容",
            "新建工程或全新模块时，可在单轮中一次性并发调用多个 write_file 创建全部基础文件与测试套件，然后统一进行验证",
            "代码写入后使用 execute_command 验证（如 pytest 或 npm test 或 npm run build）",
            "优先使用项目已有的工具和依赖，不引入冗余第三方库",
            "如果验证失败，根据报错信息修正后重新验证，直到通过",
            "修改完成后，用 git_status 总结改动，询问用户是否需要提交",
        ],
        "warnings": [
            "禁止在未读取已有文件的情况下直接覆写修改",
            "在已有项目排查修复问题时，禁止一次修改多个不相关的业务文件，应遵循最小修改原则",
        ...
        "execute_workflow": [
            ...
            "- pytest / python tests.py / python -m pytest — 运行 Python 测试",
