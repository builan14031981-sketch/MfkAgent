        ...
        self._action_patterns: List[Tuple[str, List[str]]] = [
            (
                ...
                [
                    ...
                    # 实时性信息类（避免裸名词误判 general_chat）：
                    # 行情/股价/大盘/指数/涨幅/新闻/资讯/汇率等名词本身即指向查询，直接命中；
                    # 天气类不在此列（"今天天气怎么样"属 general_chat，由 policy 实时约束兜底确保走搜索）
                    r"行情", r"股票", r"股价", r"大盘", r"指数", r"涨幅", r"跌幅",
                    r"汇率", r"金价", r"油价", r"新闻", r"资讯",
                    r"查.*行情", r"查.*股价", r"查.*股票", r"查.*大盘", r"查.*指数",
                    r"查.*汇率", r"查.*新闻", r"查.*资讯",
                    r".*涨停", r".*跌停", r"实时行情", r"今天.*涨", r"今天.*跌",
                ],
            ),
            (
                "system_diagnosis",
