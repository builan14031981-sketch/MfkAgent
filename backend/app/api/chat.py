...
def _get_memory_settings(chat_id: int) -> dict:
    """读取记忆三开关（读闸/写闸/提示）与权限模式。

    记忆三开关仍为全局设置（fail-open）；权限模式改为会话级：
    取 chats.permission_mode，缺省为 standard（无全局默认）。
    """
    ...
    try:
        ...
        for row in db.query(Setting).filter(
            Setting.key.in_(["memory_read_enabled", "memory_write_enabled", "memory_alert"])
        ).all():
        ...
        if chat and chat.permission_mode in ("safe", "standard", "autonomous"):
        ...
        return {
            ...
            "autonomous": permission_mode == "autonomous",
        }
    finally:
        db.close()
...
class ChatCreate(BaseModel):
    ...
    roundtable_config: Optional[dict] = None  # 圆桌模式配置
...
class MessageResponse(BaseModel):
    ...
    created_at: datetime
    agent_id: Optional[str] = None  # 圆桌模式：消息发送者
...
def _chat_to_response(chat) -> ChatResponse:
    ...
    return ChatResponse(
        ...
        roundtable_config=chat.roundtable_config if chat.roundtable_config else None,
...
async def create_chat(chat: ChatCreate):
    ...
    try:
        ...
        personality_level = chat.personality_level
        agent_model = None
        if (personality_level is None or chat.model is None) and agent_id:
            agent_ctx = db.query(Agent).filter(Agent.agent_id == agent_id).first()
            if agent_ctx:
                if personality_level is None:
                    personality_level = agent_ctx.default_personality_level
                # Agent ORM 模型未定义 model 字段，用原生 SQL 查询（数据库表有该列）
                if chat.model is None:
                    row = db.execute(text("SELECT model FROM agents WHERE agent_id = :aid"), {"aid": agent_id}).fetchone()
                    if row and row[0]:
                        agent_model = row[0]

        # 模型快照：request 未显式提供 model 时，从 Agent.model 快照（实现按 Agent 路由模型）。
        # Agent.model 为 NULL 时继续使用全局默认模型。
        model = chat.model or agent_model or _get_default_model()
        ...
        db_chat = Chat(
            ...
            roundtable_config=chat.roundtable_config,
        )
        db.add(db_chat)
        db.commit()
        db.refresh(db_chat)
...
async def update_chat(chat_id: int, update: ChatUpdate):
    ...
    try:
        ...
        if update.permission_mode is not None:
            chat.permission_mode = update.permission_mode
        ...
        has_content_change = (
            ...
            or update.permission_mode is not None
...
async def clear_chat_messages(chat_id: int):
    """清空整个会话的消息记录（用于「清除上下文」真实清空对话历史）。

    仅删除 Message，保留 Chat 元数据（标题 / 项目绑定 / 上下文文件）不变。
    """
    db = SessionLocal()
    try:
        chat = db.query(Chat).filter(Chat.id == chat_id).first()
        ...
        to_delete = db.query(Message).filter(Message.chat_id == chat_id).all()
        for m in to_delete:
            db.delete(m)
        chat.updated_at = datetime.utcnow()
        db.commit()
...
class SendRequest(BaseModel):
    ...
    # Phase H: checkpoint 血缘 — 断点续跑时传父 run id（仅记录追溯，不影响执行行为）
...
async def send_message(chat_id: int, request: SendRequest):
    ...
    try:
        ...
        # ──── 圆桌模式（Roundtable）：多 Agent 同会话讨论 ────
        is_roundtable = (chat.mode == "roundtable") or (chat.roundtable_config and chat.roundtable_config != "{}")
        if is_roundtable:
            from app.core.roundtable_runtime import RoundtableRuntime, RoundtableConfig
            import json as _json

            rt_raw = {}
            if chat.roundtable_config:
                try:
                    rt_raw = _json.loads(chat.roundtable_config) if isinstance(chat.roundtable_config, str) else chat.roundtable_config
                except Exception:
                    rt_raw = {}

            rt_default_model = request.model or chat.model or "gpt-4o-mini"
            rt_config = RoundtableConfig(rt_raw, default_model=rt_default_model)
            if request.temperature is not None:
                rt_config.temperature = request.temperature
            if request.max_tokens:
                rt_config.max_tokens = request.max_tokens

            if not rt_config.agents:
                db.close()
                raise HTTPException(status_code=400, detail="圆桌模式未配置参与的 Agent")
            ...
            async def _background_roundtable():
                try:
                    ...
                    logger.info("Roundtable cancelled: chat_id=%s", chat_id)
                    raise
...
async def send_message_stream(chat_id: int, request: SendRequest):
    ...
    try:
        ...
        if is_roundtable:
            ...
            async def _background_roundtable():
                try:
                    rt = RoundtableRuntime(chat_id=chat_id, config=rt_config, user_content=request.content)
                    def _emit(event):
                        try: run.queue.put_nowait(event)
...
# Phase E3: ChatContextBuilder 统一组装（AgentContext + system prompt + messages + 参数）
    ...
    async def generate():
        ...
        BATCH_MAX_CHARS = 200
        BATCH_INTERVAL = 0.02
        # Phase H: keepalive 心跳 — 后台任务长时间静默（工具执行/模型思考）时，
        # 周期性输出心跳事件防止代理/网关超时断开 SSE
        HEARTBEAT_INTERVAL = 15.0
...
async def list_chat_runs(chat_id: int):
    """Phase H: checkpoint 查看 — 返回会话的 AgentRun 历史（含 parent_run_id 血缘）。

    供前端"断点/重跑"入口读取：status 为 completed 的 run 可作为断点，
    续跑时把其 run_id 作为 SendRequest.parent_run_id 传入即建立血缘。
    """
    from app.models.agent import AgentRun as _AgentRunModel
    ...
    try:
        ...
        if not chat:
            raise HTTPException(status_code=404, detail="Chat not found")
        runs = (
            db.query(_AgentRunModel)
            .filter(_AgentRunModel.chat_id == chat_id)
            .order_by(_AgentRunModel.id.desc())
            .limit(50)
            .all()
        )
        return {
...
async def serve_attachment_file(chat_id: int, path: str):
    ...
    Query params:
        path: 附件相对路径或绝对路径（如 .mfkagent/uploads/xxx.png 或全局上传路径）
    
    安全：支持项目内 .mfkagent/uploads/ 与全局 data/uploads/ 目录的安全访问。
    """
    ...
    try:
        ...
        global_upload_dir = os.path.normpath(os.path.join(backend_root, "data", "uploads"))

        abs_path = None

        # 候选 1：绝对路径直接访问
        if os.path.isabs(path):
            norm_cand = os.path.normpath(path)
        ...
        if not abs_path and project_path:
            ...
            norm_cand = os.path.normpath(os.path.join(project_path, path))
            if norm_cand.startswith(norm_proj) and os.path.isfile(norm_cand):
                abs_path = norm_cand

        # 候选 3：相对于 backend 根目录（如 data/uploads/453/xxx.png）
        if not abs_path:
            norm_cand = os.path.normpath(os.path.join(backend_root, path))
            if norm_cand.startswith(global_upload_dir) and os.path.isfile(norm_cand):
                abs_path = norm_cand

        # 候选 4：当前 chat_id 专属上传目录下的文件名匹配
        if not abs_path:
            base_name = os.path.basename(path)
            chat_upload_dir = os.path.join(global_upload_dir, str(chat_id))
            norm_cand = os.path.normpath(os.path.join(chat_upload_dir, base_name))
            if os.path.isfile(norm_cand):
                abs_path = norm_cand
...
@router.get("/{chat_id}/generated_image")
async def serve_generated_image(chat_id: int, path: str):
    """提供文生图工具生成的图片供前端渲染（Phase H）。

    Query params:
        path: 项目内相对路径（如 output/generated_images/qwen_image_x.png）

    安全：仅允许访问项目路径下的 output/generated_images/ 目录。
    """
    import mimetypes
    from fastapi.responses import FileResponse
    ...
    try:
        ...
        # 安全检查：仅允许 output/generated_images/ 下的文件
        normalized = os.path.normpath(path).replace("\\", "/")
        if not normalized.startswith("output/generated_images/"):
            raise HTTPException(status_code=403, detail="仅允许访问文生图输出目录下的文件")
