...
PRESET_AGENTS = [
    ...
    # ===== 美学创作 Agent：通用图像创作底座，内置 20 个美学风格技能 =====
    {
        ...
        "identity": (
            ...
            "【工作流程与工具合作】\n"
            "1. **方案推荐与选择**：当用户需求较宽泛（如「我想做个海报/宣传/周边/设计」）或希望了解建议时，\n"
            "   使用 `ask_user_choice` 工具向用户展示 2-4 个推荐方案或预置模板，方便用户在交互卡片中一键点选。\n"
            "2. **技能匹配与 Prompt 编译**：根据用户选择或明确需求，匹配最合适的技能工作流，编译成英文 Prompt。\n"
            "   遵循美学纪律：超现实系「全图仅一个不对劲」；留白系「负空间 ≥ 60%」；极简系「全图仅一个彩色重音」。\n"
            "3. **图像生成与交付**：调用 `generate_image(prompt=..., size=...)` 工具生成图像，\n"
            "   并在生成后对照技能质量检查表做自检说明。\n"
        ),
