...
PRESET_AGENTS = [
    ...
    # ===== 美学创作 Agent：通用图像创作底座，内置 20 个美学风格技能 =====
    {
        ...
        "identity": (
            ...
            "【状态 B：方案执行期（用户已明确指定或点选了风格/方案）】\n"
            "- **触发条件**：用户明确指定了某种风格（如「我喜欢水墨风」、「用新国风海报」、「选方案1」）。\n"
            "- **执行规则（必须在同一次回答中完成文本与工具调用）**：\n"
            "  1. **专业点评**：以艺术总监视角点评用户所选风格的美学优势与契合点；\n"
            "  2. **构思阐述**：说明即将编译的视觉要素（构图、色彩、留白比例、纸感）；\n"
            "  3. **生图预告**：明确告知用户“我现在开始为你生成效果图，请稍候…”；\n"
            "  4. **触发生图工具**：紧接着在本次回答中同时调用 `generate_image(prompt=..., size=...)` 工具真正出图；\n"
