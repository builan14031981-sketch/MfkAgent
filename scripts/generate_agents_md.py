#!/usr/bin/env python3
# -*- coding: utf-8 -*-
"""生成仓库根 AGENTS.md —— 面向未来 AI 助手的项目指引。

用法（仓库任意位置执行均可）：
    python scripts/generate_agents_md.py [--output <path>]

    --output <path>  输出到指定路径（默认仓库根 AGENTS.md），便于预览/测试无副作用。

说明：
    - 纯标准库实现，零第三方依赖，在"干净环境"（无 venv / 无依赖）下也能直接运行。
    - 默认输出到仓库根 AGENTS.md；本脚本是唯一事实来源，重跑即幂等覆盖。
    - 生成前对正文引用的关键路径做存在性校验：缺失时打印警告（不阻断生成），
      便于及时发现仓库现状与文档内容的漂移。
"""
import os
import sys
...
# AGENTS.md 正文引用的关键路径（提交态应存在；缺失则告警）。
# 注意：backend/.venv 属运行时环境（已 gitignore，不一定存在），不列入校验。
_REQUIRED_PATHS = [
    "backend/main.py",
    "backend/requirements.txt",
    "backend/tests/conftest.py",
    "backend/tests/pytest.ini",
    "backend/app/core/tool_runtime/runtime.py",
    "backend/app/api/chat.py",
    "frontend/package.json",
    "frontend/src/app/page.tsx",
    "frontend/electron/main.js",
    "scripts/build_greetings.py",
    "backend_guardian.ps1",
    "start-desktop.bat",
]

# fmt: off
_CONTENT = """# AGENTS.md — MfkAgent 仓库指引
...
def _validate_paths() -> int:
    """校验正文引用的关键路径是否存在（提交态应存在），返回缺失数量。"""
    missing = [p for p in _REQUIRED_PATHS if not os.path.exists(os.path.join(REPO_ROOT, p))]
    for p in missing:
        print(f"[generate_agents_md] 警告：AGENTS.md 引用的路径不存在，请核对正文: {p}", file=sys.stderr)
    return len(missing)


def generate(output_path: str = OUTPUT_PATH) -> int:
    """生成 AGENTS.md 内容到 output_path（UTF-8 / LF），返回 0。"""
    missing = _validate_paths()
    with open(output_path, "w", encoding="utf-8", newline="\n") as f:
        f.write(_CONTENT)
    print(f"[generate_agents_md] 已生成 {output_path}")
    if missing:
        print(f"[generate_agents_md] 完成（{len(missing)} 个引用路径缺失，见上方警告，不影响本次生成）")
    return 0
...
def main(argv: Optional[List[str]] = None) -> int:
    ...
    if "--output" in args:
        ...
        if idx + 1 >= len(args):
            print("[generate_agents_md] 用法: python scripts/generate_agents_md.py [--output <path>]", file=sys.stderr)
            return 2
        output = args[idx + 1]
